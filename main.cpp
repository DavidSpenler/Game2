...
void create_rooms(vector<vector<int>> room_grid, vector<vector<vector<int>>> room_spread, vector<vector<int>> doors, int num_rooms, vector<Room>& rooms, int floor, int max_floor, vector<Room*>& stair_rooms) {
	...
	for (int y=0;y<room_grid.size();++y) {
		for (int x=0;x<room_grid[0].size();++x) {
			...
			if (x==0) {
				if (y==0 && !(stair_rooms[0] == NULL && floor == max_floor)) {
					...
					if (stair_rooms[0] == NULL && floor != max_floor) {
						this_p_room = &rooms[room_grid[y][x]-1+index_offset];
						stair_rooms[0] = this_p_room;
					}
					else {
						rooms[room_grid[y][x]-1+index_offset].add_stair_connection(stair_rooms[0]);
						this_p_room = &rooms[room_grid[y][x]-1+index_offset];
						stair_rooms[0]->add_stair_connection(this_p_room);
						stair_rooms[0] = NULL;
					}
				}
				else if (y+1==room_grid.size() && floor != 1 && !(stair_rooms[1] == NULL && floor == max_floor)) {
					rooms[room_grid[y][x]-1+index_offset].make_sprite(texture_stairs,50,50,x*500+50,500*(y+1)-100,3);
					FloatRect stair_hit_box(Vector2f(x*500+74,500*(y+1)-74),Vector2f(2,2));
					rooms[room_grid[y][x]-1+index_offset].add_stair_hit_box(stair_hit_box);
					if (stair_rooms[1] == NULL && floor != max_floor) {
						cout << "bottom left set" << endl;
						this_p_room = &rooms[room_grid[y][x]-1+index_offset];
						...
						stair_rooms[1] = NULL;
						cout << "bottom left resolved" << endl;
					}
				}
			}
			else if (x+1==room_grid[0].size()) {
				if (y==0 && floor != 1 && !(stair_rooms[2] == NULL && floor == max_floor)) {
					rooms[room_grid[y][x]-1+index_offset].make_sprite(texture_stairs,50,50,(x+1)*500-100,500*y+50,3);
					FloatRect stair_hit_box(Vector2f((x+1)*500-76,500*y+74),Vector2f(2,2));
					rooms[room_grid[y][x]-1+index_offset].add_stair_hit_box(stair_hit_box);
					if (stair_rooms[2] == NULL && floor != max_floor) {
						cout << "top right set" << endl;
						this_p_room = &rooms[room_grid[y][x]-1+index_offset];
						...
						stair_rooms[2] = NULL;
						cout << "top right resolved" << endl;
					}
				}
				else if (y+1==room_grid.size() && !(stair_rooms[3] == NULL && floor == max_floor)) {
					...
					rooms[room_grid[y][x]-1+index_offset].add_stair_hit_box(stair_hit_box);
					if (stair_rooms[3] == NULL && floor != max_floor) {
						this_p_room = &rooms[room_grid[y][x]-1+index_offset];
...
void create_locks(vector<Room>& rooms) {
	vector<Room*> candidates;
	for (int r=0;r<rooms.size();++r) {
		if (rooms[r].door_connections.size() == 1 && rooms[r].stair_connections.size() == 0) {
			candidates.push_back(&rooms[r]);
		}
	}
}
...
vector<Room> generate_map(int num_floors) {
	vector<Room> rooms = {};
	rooms.reserve(100);
	vector<Room*> stair_rooms = {};
	stair_rooms.reserve(4);
	vector<Room*> isolated_rooms;
	for (int s=0;s<4;++s) {
		stair_rooms.push_back(NULL);
	}
	...
	for (int i=0;i<rooms.size();++i) {
		...
		if (rooms[i].doors.size() == 1 && rooms[i].stairs.size() == 0) {
			bool recur = true;
			Room this_room = *rooms[i].door_connections[0];
			Room cache_room = NULL;
			Room last_room = NULL;
			
			while (true) {
				if (this_room.doors.size() == 2 && this_rooms.stairs.size() == 0) {
					cache_room = (*(this_room.door_connections[0])==last_room) ? this_room.door_connections[1]:this_room.door_connections[0]
					last_room = this_room;
					this_room = cache_room;
				}	
				else {
					break;
				}
		...
		while (recur == true) {
			...
			if (this_room.doors.size() == 1 && rooms[i].stairs.size() == 0) {
				this_room = *this_room.door_connections[0];
